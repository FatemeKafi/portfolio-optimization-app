...
def run_risk_questionnaire():
    """
    Runs the risk assessment questionnaire and returns the calculated risk level
    ...
    total_score = 0
    answer_q11_idx = None # Store index of chosen option for Q11
    answer_q12_idx = None # Store index of chosen option for Q12
    ...
    risk_level_for_portfolio = None # Initialize to None

    # Map total_score to risk levels 2-9
    # This mapping is an assumption based on the provided score ranges and portfolio risk levels
    # Please adjust if your specific mapping is different.
    ...
    # Display ESG and Strategy preferences based on stored answers
    esg_message = ""
    # Assuming 'Yes' is the first option (index 0) for Q11
    if answer_q11_idx == 0 and questions[11]["options"][answer_q11_idx] == "Yes, I want a portfolio with sustainable investments":
        esg_message = "You prefer investing in a sustainable portfolio."
    
    strategy_message = ""
    # Assuming 'Yes' is the first option (index 0) for Q12
    if answer_q12_idx == 0 and questions[12]["options"][answer_q12_idx] == "Yes, I want an active strategy":
        strategy_message = "You prefer an active investment strategy."
    
    # Print the final result in a clearer way
    ...
    return risk_level_for_portfolio # Return the determined risk level
...
# --- Plotting Functions (with console output) ---
...
if __name__ == "__main__":
    ...
    # Run the risk questionnaire to get the user's risk level
    determined_risk_level = run_risk_questionnaire()

    if determined_risk_level is None:
        print("\nCould not determine a valid risk level from the questionnaire.")
        print("Please restart and ensure your answers fall within a defined risk profile.")
    elif determined_risk_level not in portfolio_data:
        print(f"\nDetermined risk level {determined_risk_level} does not have a defined portfolio.")
        print("Please ensure your risk level (2-9) corresponds to an available portfolio.")
    else:
        print(f"\n--- Proceeding with Portfolio Analysis for Risk Level: {determined_risk_level} ---")
        ...
        if all_historical_data.empty:
            ...
            # Use the determined risk level from the questionnaire
            selected_portfolio = portfolio_data[determined_risk_level]
            ...
            print(f"\n--- Analyzing Portfolio for Risk Level {determined_risk_level} ---")

            # --- 1. Plot Category Allocation ---
            plot_pie_chart_with_details(category_allocation, f'Portfolio Allocation by Category (Risk Level {determined_risk_level})')
                ...
                if category_total_weight_in_portfolio > 0.001:
                    if len(allocation_dict) > 1:
                        ...
                        plot_pie_chart_with_details(normalized_allocation_dict,
                                                    f'Detailed Allocation within {category} (Risk Level {determined_risk_level})')
                    # For single asset categories, we just print the detail, no separate pie chart
                        ...
                        print(f"\n--- Detailed Allocation within {category} (Risk Level {determined_risk_level}) Details ---")
            ...
            print(f"\n--- Specific ETFs and Their Percentages for Risk Level {determined_risk_level} ---")
            ...
            print(f"\n--- Portfolio Performance ({analysis_start_date} to {analysis_end_date}) for Risk Level {determined_risk_level} ---")
            ...
            if portfolio_cumulative_growth_indexed is not None and not portfolio_cumulative_growth_indexed.empty:
                excel_filename = f"portfolio_{determined_risk_level}_cumulative_growth_{analysis_start_date}_to_{analysis_end_date}.xlsx"
                try:
